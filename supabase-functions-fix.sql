...
BEGIN
    ...
    SELECT 
        ...
        md.unique_customers,
        md.average_transaction_value,
        md.total_points_earned,
        md.total_points_redeemed
    FROM merchant_dashboard md
    JOIN merchants m ON md.merchant_id = m.id
    WHERE m.auth_user_id = auth_user_uuid;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Alternative: Create a simpler dashboard function that directly queries without using the view
CREATE OR REPLACE FUNCTION get_merchant_dashboard_simple(auth_user_uuid UUID)
RETURNS TABLE (
    merchant_id UUID,
    business_name VARCHAR,
    merchant_code VARCHAR,
...
GRANT EXECUTE ON FUNCTION get_merchant_dashboard_simple TO authenticated;